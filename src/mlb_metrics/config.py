...
KELLY_REFERENCE_BANKROLL = 1000  # a fixed NOTIONAL bankroll purely so game_predictions.csv's logged bet_stake_dollars/bet_profit_dollars are comparable day to day - NOT a live, compounding bankroll. A real bettor sizes their own real stakes off their own real current bankroll via scripts/recommend_bets.py's own --bankroll flag, which is completely unaffected by this constant; this one only controls what gets LOGGED for tracking purposes.
